...
## 11. النظام البصري (Design System)

### الألوان المخصصة في `globals.css`:
النظام يستخدم نظام ألوان Material Design 3 مخصص بأسماء تبدأ بـ `el-`:
- `--el-primary`: اللون الأساسي (أزرق داكن)
- `--el-secondary`: اللون الثانوي
- `--el-surface`: خلفيات الأسطح
- `--el-on-primary`, `--el-on-surface`: ألوان النص

### الخط العربي:
يستخدم `font-family: 'Segoe UI', Tahoma, 'Arial', sans-serif` مع `direction: rtl`.
