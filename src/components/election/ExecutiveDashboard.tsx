...
export default function ExecutiveDashboard() {
  ...
  return (
    <div className="flex flex-col gap-3 max-w-[1440px] mx-auto w-full">
      ...
      <section className="bg-el-surface-container-lowest border border-el-outline-variant rounded-lg p-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* الخريطة التفاعلية */}
        <div className="lg:col-span-2 flex flex-col gap-2">
          <h3 className="text-[14px] font-bold text-el-on-surface flex items-center gap-2">
            <MapPin className="w-4 h-4 text-el-primary" /> خريطة الدوائر الانتخابية التفاعلية — ذي قار
          </h3>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex justify-between items-center">
            <h3 className="text-[14px] font-bold text-el-on-surface flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-el-secondary" /> ملخص توزيع القوة جغرافياً
            </h3>
            <div className="flex gap-2 text-[10px]">
              <span className="flex items-center gap-0.5"><span className="w-2 h-2 rounded-full bg-green-500" /> قوية ({greenCount})</span>
              <span className="flex items-center gap-0.5"><span className="w-2 h-2 rounded-full bg-yellow-400" /> متأرجحة ({yellowCount})</span>
              <span className="flex items-center gap-0.5"><span className="w-2 h-2 rounded-full bg-red-400" /> ضعيفة ({redCount})</span>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto max-h-[380px] space-y-1.5 custom-scroll pr-1">
            {areaMap.map((area: any) => (
              <div
                key={area?.district || Math.random().toString()}
                className={`rounded-lg p-2.5 border transition-all hover:shadow-sm flex items-center justify-between ${
                  area?.color === 'green' ? 'border-green-300/60 bg-green-50/50 dark:bg-green-950/20' :
                  area?.color === 'yellow' ? 'border-yellow-300/60 bg-yellow-50/50 dark:bg-yellow-950/20' :
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12.5px] font-bold text-el-on-surface">{area?.district || 'غير محدد'}</span>
                </div>
                <div className="flex gap-4 items-center text-[11px] text-el-on-surface-variant font-mono">
                  <span>صافي: <b>{area?.netVotes ?? 0}</b></span>
                  <span>تأييد: <b>{area?.strength ?? 0}%</b></span>
