...
(async () => {
  ...
  const login = await req('/api/access', 'POST', { action: 'owner-login', ownerPassword: 'DhiQarOwner2026!' });
  console.log('Owner login:', login.status, login.body.substring(0, 200));
  
  if (login.status !== 200) {
    // Try as visitor
    const visitorLogin = await req('/api/access', 'POST', { action: 'login', password: 'DhiQarUser2026!' });
